...
import { addMessage, getMessagesAfterSeq, getAllMessages, getOrCreateMeeting, enableAI, disableAI } from './meetingState';
...
async function handleMessage(ws: WebSocket, message: ClientMessage): Promise<void> {
  ...
  switch (message.action) {
    case 'join': {
      ...
      const meeting = await getOrCreateMeeting(message.roomId);
    ...
    case 'chat': {
      ...
      const chatMessage = await addMessage(session.roomId, {
    ...
    case 'summarize': {
      ...
      const messages = await getAllMessages(session.roomId);
      sendToClient(ws, { type: 'error', message: '正在生成会议总结...' });

      const summary = await generateSummary(session.roomId, messages);
      const summaryMessage: ChatMessage = await addMessage(session.roomId, {
        sender: 'AI 助手',
        content: summary,
        timestamp: Date.now(),
        type: 'summary',
    ...
    case 'sync': {
      ...
      const missedMessages = await getMessagesAfterSeq(session.roomId, message.lastSeq);
...
export function broadcastToRoomSync(roomId: string, message: ServerMessage): void {
