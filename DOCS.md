...
│   ├── certs/                    # mkcert 自签证书（19447 端口）
│   ├── start.sh                  # 一键启动脚本（自动检测 IP + 重新生成证书）
│   ├── stop.sh                   # 停止脚本
│   └── .env                      # 局域网 IP / 鉴权开关
│
├── silan-asr-service/            # 后端 ASR + 会议服务（Python）
│   ├── main.py                   # 启动入口（WebSocket 19087 + Flask HTTP 19089）
│   ├── app/
...
│  Web/Jicofo/JVB/   │  │                                      │
│  Prosody           │  │  ┌─────────────┐  ┌──────────────┐   │
│  JWT 鉴权(HS256)    │  │  │ WS 网关 19087 │  │ Flask 19089   │   │
│  19447/19107/5222   │  │  │  会议房间     │  │  HTTP API    │   │
└────────────────────┘  │  │  ASR 音频     │  │  Bot Spawn   │   │
                        │  └──────┬──────┘  └──────┬───────┘   │
                        │         │                │           │
                        │  ┌──────▼──────┐  ┌──────▼──────┐    │
...
**加载流程**：
1. 动态加载 `https://{host}:19447/external_api.js`（`useJitsiApi.ts`）
...
| 连接 | 用途 | 端口 | 生命周期 |
|---|---|---|---|
| `useWebSocket` | 会议房间消息（聊天 / 转写 / AI 状态 / 同步） | 19087 | 加入会议后持续 |
| `audioCapture` / `ParticipantAudioReceiver` | ASR 音频上传（audio_chunk） | 19087 | 录制期间 |
...
python main.py --device cpu --port 19087 --host 0.0.0.0

# 3. 启动前端
cd frontend && npm install && npm run dev
# 浏览器打开 https://localhost:19307
```
...
| 端口 | 用途 |
|---|---|
| 19447 | Jitsi Web（HTTPS，mkcert 自签证书） |
| 19307 | Vite 前端开发服务器（HTTPS，mkcert 自签证书） |
| 19087 | 后端 WebSocket（会议房间 + ASR 音频 + Bot recorder） |
| 19089 | 后端 Flask HTTP（API / 健康检查） |
| 19107/UDP | Jitsi JVB 媒体流 |
| 5222 | Prosody XMPP |
| 6379 | Redis（状态持久化） |
