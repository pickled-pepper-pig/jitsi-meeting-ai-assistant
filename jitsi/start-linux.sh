#!/bin/bash
# Jitsi 本地开发环境启动脚本（Linux 版本）
# 使用方法：./start.sh
#
# 与 macOS 版 start.sh 的区别：
#   - 获取局域网 IP 使用 `ip route` / `hostname -I` 而非 `ipconfig getifaddr`
#   - sed 原地修改使用 `sed -i` 而非 `sed -i ''`（BSD/GNU 差异）
#   - 其余逻辑与 macOS 版一致
...
get_local_ip() {
    ...
    # 方式1：ip route（最准确，取默认路由出口网卡的 IP）
    ...
    # 方式2：hostname -I（Ubuntu/Debian 常见，返回所有非环回 IP，取第一个）
    ...
    if command -v ifconfig > /dev/null 2>&1; then
        ip=$(ifconfig 2>/dev/null | grep -oP 'inet (?:addr:)?\K[\d.]+' | grep -v '^127\.' | head -n1)
        if [ -n "$ip" ]; then
            echo "$ip"
            return 0
        fi
...
if [ -f .env ]; then
    ...
    echo "⚠️  未找到 .env 文件，请先创建（可从 .env.example 复制）"
...
# 启动 Jitsi 服务
echo "🚀 启动 Jitsi 服务..."
echo "   - Web:  http://localhost:19407"
echo "   - HTTPS: https://localhost:19447 (自签名证书，浏览器会警告)"
echo ""
...
echo ""
echo "  访问地址："
echo "    - HTTP:  http://localhost:19407"
echo "    - HTTPS: https://localhost:19447"
echo "    - 局域网: https://$CURRENT_IP:19447"
echo ""
echo "  注意事项："
